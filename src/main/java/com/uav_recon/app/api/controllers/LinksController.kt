...
class LinksController(
    ...
    fun inspection(@PathVariable userId: Long,
    ...
    fun observation(@PathVariable userId: Long,
                    @PathVariable inspectionId: String,
    ...
    fun observationDefect(@PathVariable userId: Long,
                          @PathVariable inspectionId: String,
                          @PathVariable observationId: String,
    ...
    @GetMapping("/{inspectionId}/{observationId}/{observationDefectId}/{photoId}")
    @Throws(IOException::class)
    fun getImageAsByteArray(@PathVariable userId: Long,
                            @PathVariable inspectionId: String,
                            @PathVariable observationId: String,
                            @PathVariable observationDefectId: String,
        ...
        if (optional.isPresent) {
            val observations = optional.get().observations?.filter { o -> o.uuid == observationId }
    ...
    private fun populateModel(userId: Long, inspectionId: String, model: Model, session: HttpSession,
                              observationId: String? = null, observationDefectId: String? = null
    ) {
        val title: String?
        val header: String?
        val items: List<Any>?
        ...
        val inspectionUrl = "/datarecon-links/$userId/$inspectionId"
        ...
        if (observationId == null) {
            ...
            val observations = inspectionDto.observations?.filter { o -> o.uuid == observationId }
            if (observations == null || observations.isEmpty()) {
                model.addAttribute(errorMessageAttribute, "Invalid observation uuid")
                return
            }

            if (observations.size > 1) {
                model.addAttribute(errorMessageAttribute,
                        "There are more then one observation with uuid $observationId. " +
            ...
            if (observationDefectId == null) {
                ...
                val defects = observation.observationDefects?.filter { d -> d.uuid == observationDefectId }
                ...
                parents.add(Parent("$observationUrl/$observationDefectId",
                        "Observation defect $observationDefectId"
                ))
                title = "Observation defect $observationDefectId"
                header = "Photos"
                items = defect.photos?.map { p -> Photo("${p.uuid}.jpeg", p.name) }
    ...
    class Photo(val uuid: String, val id: String?)
